...
- [ ] **用户审阅最终成果并明确批准 push/创建 PR。** 本项是用户原始授权边界，
  不是工具或技能附加的审批要求；目前未进行任何外部发布。
- [ ] 用户确认本源溯知报名/队伍信息有效（代码不包含个人信息，也未代填报名）。
- [ ] 提交前再次刷新开放 PR 和 upstream/develop；若新出现高度重合投稿，先调整。
- [ ] 在用户个人 GitHub 空间 fork 官方仓库，配置 `origin`（当前只有 `upstream`）。
- [ ] 确认基线更新无冲突，必要时合并 develop 并重复相关验证。
