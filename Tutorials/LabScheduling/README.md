...
增强版入口：[连锁改约演示、安全化简证明与 120 次冻结评测](ENHANCEMENTS.md)。
已实现 XY 线路精简、独立冲突分量求解及原问题回填，保留所有失败实例。
最新入口：[维护者与评委审阅路线](REVIEW_GUIDE.md)、
[36 个异构实例与完整量子业务取舍](EVALUATION_V2.md)。
同总预算及化简后随机对照见[补充评价](MATCHED_BUDGET.md)，包括随机持平/胜出的结果。
可选[无支持候选化简](ARC_PRUNING.md)在保留全部可行排程的前提下，将一个
18 比特超限组件降到 12 比特；提供完整删除证据、真实 CPU 结果和不可行反例。
已有取舍演示支持 `--sensitivity`，展示改约成本变化如何改变最优决策，
见[成本权重分析](EVALUATION_V2.md#改约成本敏感性什么时候值得使用备用设备)。
下图是原版本的历史开发基线，新版结果见上述增强教程。

![历史开发基线：多种子可行概率与实验室排程](results/overview.svg)
...
- 精确穷举 API 最多枚举 1,000,000 个组合，超限报错；报告中跳过超预算枚举，
  另行运行原始 MILP，只有 optimal 状态用作最优证书。此应用面向小范围
